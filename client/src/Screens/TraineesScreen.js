...
import { listTrainees } from '../actions/staffActions';
import search from '../icons/search.svg';
import  url  from '../icons/return.svg'

function TraineesScreen(props){
    ...
    const {trainees, loading, error} = traineeList;
    const dispatch = useDispatch();
    
    useEffect(() => {
        dispatch(listTrainees(name));
        return () => {
            //
        };
    }, [name])
    ...
    const submitHandler = (e) => {
        e.preventDefault();
        dispatch(listTrainees(name, searchKeyword))
    }
                ...
                <Form onSubmit={submitHandler} className="d-flex">
                    <FormGroup className="d-flex">
                        ...
                        <Button color="success" className="border ml-5">
                            <Link to="/createTrainee" style={{textDecoration:'none', color: 'white'}}>
                                Create
                            </Link>
                        </Button>
                    </FormGroup>
                        ...
                        userInfo.isAdmin || userInfo.isTraining  ?(
                            <>
                            {
                                trainees.map(trainee => 
                                    <Col lg='3' md='4' sm='6' xs='6' key={trainee._id}>
                                        <Card>
                                            <Link to={'/trainee/'+ trainee._id}>
                                            <div style={{backgroundColor: '#646566', height:'22vh'}}/>
                                                <CardBody>
                                                    <CardTitle className="font-weight-bold">{trainee.name}</CardTitle>
                                                </CardBody>
                                            </Link>
                                        </Card>
                                    </Col>
